...
AUDIO_CNN_FEATURES: List[Dict] = [
    {
        "feature_id": "ACF001",
        "name": "audio_cnn2048_stft128x96",
        "embedding_dim": 2048,
...
AUDIO_BM_EXPERIMENTS: List[Dict] = [
    {
        "id": "AF005",
        "name": "standard_audio_cnn2048_h4_lc5_e180",
        "feature_id": "ACF001",
        "embedding_dim": 2048,
        "hidden_factor": 4.0,
        "label_copies": 5,
    ...
    {
        "id": "AF006",
        "name": "standard_audio_cnn2048_h6_lc5_e180",
        "feature_id": "ACF001",
        "embedding_dim": 2048,
        "hidden_factor": 6.0,
        "label_copies": 5,
    ...
    {
        "id": "AF007",
        "name": "standard_audio_cnn4096_h4_lc5_e180",
        "feature_id": "ACF002",
        "embedding_dim": 4096,
        "hidden_factor": 4.0,
        "label_copies": 5,
    ...
    {
        "id": "AF008",
        "name": "standard_audio_cnn4096_h6_lc5_e180",
        "feature_id": "ACF002",
        "embedding_dim": 4096,
        "hidden_factor": 6.0,
        "label_copies": 5,
...
def audio_cnn_paths(root: Path, feature: Dict) -> Dict[str, Path]:
...
def ensure_audio_cnn_feature(root: Path, args: argparse.Namespace, feature: Dict, source_npz: Path) -> Path:
    paths = audio_cnn_paths(root, feature)
    if paths["npz"].exists() and paths["summary"].exists() and not args.force_features:
        print(f"SKIP audio CNN feature: {paths['npz']}", flush=True)
        return paths["npz"]
    cmd = [
        str(args.python_bin),
    ...
    print(f"\n[{now_text()}] TRAIN AUDIO CNN FEATURE {feature['feature_id']} -> {paths['npz']}", flush=True)
...
def write_log(root: Path, teacher_summaries: List[Dict], bm_results: List[Dict]) -> None:
    ...
    (root / "vggsound_full_audio_cnn_bm_log.md").write_text(text, encoding="utf-8")


def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run full VGGSound audio CNN embedding + audio BM experiments.")
    ...
    p.add_argument("--teacher_batch_size", type=int, default=256)
    p.add_argument("--teacher_eval_batch_size", type=int, default=256)
...
def main() -> None:
    ...
    for feature in features:
        feature_npz = ensure_audio_cnn_feature(root, args, feature, source_npz)
        feature_paths[feature["feature_id"]] = feature_npz
        summary_path = audio_cnn_paths(root, feature)["summary"]
    ...
    write_log(root, teacher_summaries, bm_results)
    print("VGGSound full audio CNN BM sweep finished.", flush=True)
