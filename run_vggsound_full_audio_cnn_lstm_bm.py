...
AUDIO_CNN_LSTM_FEATURES: List[Dict] = [
    {
        "feature_id": "ACL001",
        "name": "audio_cnnlstm2048_stft128x96",
        "embedding_dim": 2048,
    ...
    {
        "feature_id": "ACL002",
        "name": "audio_cnnlstm4096_stft128x96",
        "embedding_dim": 4096,
        "normalize": "per_dim_zscore_sigmoid",
        "teacher_epochs": 50,
        "seed": 123,
...
AUDIO_BM_EXPERIMENTS: List[Dict] = [
    {
        "id": "AF009",
        "name": "standard_audio_cnnlstm2048_h4_lc5_e180",
        "feature_id": "ACL001",
        "embedding_dim": 2048,
        "hidden_factor": 4.0,
        "label_copies": 5,
    ...
    {
        "id": "AF010",
        "name": "standard_audio_cnnlstm2048_h6_lc5_e180",
        "feature_id": "ACL001",
        "embedding_dim": 2048,
        "hidden_factor": 6.0,
        "label_copies": 5,
    ...
    {
        "id": "AF011",
        "name": "standard_audio_cnnlstm4096_h4_lc5_e180",
        "feature_id": "ACL002",
        "embedding_dim": 4096,
        "hidden_factor": 4.0,
        "label_copies": 5,
    ...
    {
        "id": "AF012",
        "name": "standard_audio_cnnlstm4096_h6_lc5_e180",
        "feature_id": "ACL002",
        "embedding_dim": 4096,
        "hidden_factor": 6.0,
        "label_copies": 5,
...
def audio_cnn_lstm_paths(root: Path, feature: Dict) -> Dict[str, Path]:
...
def ensure_audio_cnn_lstm_feature(root: Path, args: argparse.Namespace, feature: Dict, source_npz: Path) -> Path:
    paths = audio_cnn_lstm_paths(root, feature)
    if paths["npz"].exists() and paths["summary"].exists() and not args.force_features:
        print(f"SKIP audio CNN-LSTM feature: {paths['npz']}", flush=True)
        return paths["npz"]
    cmd = [
        str(args.python_bin),
    ...
    print(f"\n[{now_text()}] TRAIN AUDIO CNN-LSTM FEATURE {feature['feature_id']} -> {paths['npz']}", flush=True)
...
def write_log(root: Path, teacher_summaries: List[Dict], bm_results: List[Dict]) -> None:
    ...
    for s in teacher_summaries:
        teacher_rows.append(
            "| {experiment_id} | {embedding_dim} | {lstm_hidden} | {best_epoch} | {top1} |".format(
                experiment_id=s.get("experiment_id", ""),
                embedding_dim=s.get("embedding_dim", ""),
                lstm_hidden=s.get("lstm_hidden", ""),
    ...
    (root / "vggsound_full_audio_cnn_lstm_bm_log.md").write_text(text, encoding="utf-8")


def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run full VGGSound audio CNN-LSTM embedding + audio BM experiments.")
    p.add_argument("--root", type=str, default=".")
    p.add_argument(
        "--source_audio_npz",
        type=Path,
        default=Path("./data_vggsound_full/features/vggsound_full_audio_stft128x96_official5s_allclasses_sr16000_n512_o353.npz"),
    )
    ...
    p.add_argument("--only_2048", action="store_true")
    ...
    p.add_argument("--lstm_input_dim", type=int, default=512)
    ...
    p.add_argument("--teacher_eval_batch_size", type=int, default=192)
...
def main() -> None:
    ...
    features = AUDIO_CNN_LSTM_FEATURES
    if args.only_2048:
        features = [f for f in features if f["embedding_dim"] == 2048]
    if args.only_4096:
    ...
    for feature in features:
        feature_npz = ensure_audio_cnn_lstm_feature(root, args, feature, source_npz)
        feature_paths[feature["feature_id"]] = feature_npz
        summary_path = audio_cnn_lstm_paths(root, feature)["summary"]
        if summary_path.exists():
            teacher_summaries.append(json.loads(summary_path.read_text(encoding="utf-8")))
        write_log(root, teacher_summaries, bm_results)

    if not args.skip_bm:
        bm_experiments = AUDIO_BM_EXPERIMENTS
        if args.only_2048:
            bm_experiments = [e for e in bm_experiments if e["embedding_dim"] == 2048]
        if args.only_4096:
            bm_experiments = [e for e in bm_experiments if e["embedding_dim"] == 4096]
    ...
    write_log(root, teacher_summaries, bm_results)
    print("VGGSound full audio CNN-LSTM BM sweep finished.", flush=True)
