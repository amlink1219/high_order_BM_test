...
def main() -> None:
    p = argparse.ArgumentParser(description="Merge full VGGSound RGB+motion sequence feature shards.")
    ...
    shard_paths = [Path(x).resolve() for x in args.shards]
    shards = [dict(np.load(x, allow_pickle=True)) for x in shard_paths]
    if not shards:
    ...
    for path, shard in zip(shard_paths, shards):
        names = [str(x) for x in shard["class_names"].tolist()]
        if names != class_names:
            raise ValueError(f"class_names mismatch in {path}")

    payload = {
        "rgb_seq_train": concat_key(shards, "rgb_seq_train"),
        "motion_seq_train": concat_key(shards, "motion_seq_train"),
        "y_train": concat_key(shards, "y_train").astype(np.int64),
        "path_train": concat_key(shards, "path_train"),
        "rgb_seq_test": concat_key(shards, "rgb_seq_test"),
        "motion_seq_test": concat_key(shards, "motion_seq_test"),
    ...
    summary = {
        "created_at": now_text(),
        "out_npz": str(out_npz),
        "shards": [str(x) for x in shard_paths],
        ...
        "dtype": str(payload["rgb_seq_train"].dtype),
    }
