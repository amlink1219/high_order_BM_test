...
REFERENCE = {
    "audio": "AF031 paper-STFT ResNet50 LSTM4096 h6 full=44.31%",
    "video_same_dim": "VF024 / VLF002 video LSTM4096 h8 full=42.74%",
    "video_overall": "VF026 / VLF003 video LSTM8192 h6 full=42.84% but not same-dim with AF031",
}
...
EXPERIMENTS: List[Dict] = [
    {
        "id": "AV011",
        "name": "standard_avg_videolstm4096_audioaf031_lstm4096_h8_lc5_e320",
        "model_type": "standard",
    ...
    {
        "id": "AV012",
        "name": "twoport_videolstm4096_audioaf031_lstm4096_h8_g115_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 8.0,
        "gamma_h": 1.15,
        "gamma_l": 1.15,
    ...
    {
        "id": "AV013",
        "name": "twoport_videolstm4096_audioaf031_lstm4096_h8_g050_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 8.0,
        "gamma_h": 0.50,
        "gamma_l": 0.50,
    ...
    {
        "id": "AV014",
        "name": "twoport_videolstm4096_audioaf031_lstm4096_h8_g000_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 8.0,
        "gamma_h": 0.0,
        "gamma_l": 0.0,
    ...
    {
        "id": "AV015",
        "name": "twoport_videolstm4096_audioaf031_lstm4096_h6_g115_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 6.0,
        "gamma_h": 1.15,
        "gamma_l": 1.15,
...
def feature_info(feature_npz: Path) -> Dict:
    ...
    return {
        ...
        "audio_dim": int(data["audio_train"].shape[1]),
...
def ensure_aligned_feature(root: Path, args: argparse.Namespace) -> Path:
    ...
    print(f"\n[{now_text()}] ALIGN VF024-compatible video feature with AF031 audio feature -> {out_npz}", flush=True)
    run_checked(
        cmd,
        root,
        root / "runs_vggsound_full_AV011_align_af031_stdout.log",
        root / "runs_vggsound_full_AV011_align_af031_stderr.log",
        dry_run=args.dry_run,
    )
    return out_npz


def parse_experiment_filter(value: str) -> set[str] | None:
    value = value.strip()
    if not value:
        return None
...
def selected_experiments(args: argparse.Namespace) -> List[Dict]:
    keep = parse_experiment_filter(args.only)
    if keep is None:
        return EXPERIMENTS
    chosen = [exp for exp in EXPERIMENTS if exp["id"].upper() in keep]
    missing = keep.difference({exp["id"].upper() for exp in chosen})
    if missing:
        raise ValueError(f"unknown experiment IDs in --only: {sorted(missing)}")
    return chosen


def train_one(root: Path, args: argparse.Namespace, exp: Dict, feature_npz: Path, info: Dict) -> Dict:
    input_dim = int(info["input_dim"])
    audio_dim = int(info["audio_dim"])
    if audio_dim != input_dim:
        raise RuntimeError(f"aligned feature must have equal video/audio dims; got video={input_dim}, audio={audio_dim}")
...
def write_log(root: Path, feature_npz: Path, results: List[Dict]) -> None:
    rows = []
    for result in results:
        ...
        rows.append(
            "| {experiment_id} | {model_type} | {input_dim} | {hidden_dim} | {total_pbits} | {best_epoch} | {best} | {full} |".format(
    ...
    text = "\n".join(
        [
            ...
            f"Updated: {now_text()}",
            "",
            f"Aligned feature: `{feature_npz}`",
            "",
            "Reference before this branch:",
            "",
            f"- audio-only BM: {REFERENCE['audio']}",
            f"- video-only BM, same 4096-d visible feature: {REFERENCE['video_same_dim']}",
            f"- video-only BM, overall best: {REFERENCE['video_overall']}",
            "",
    ...
    (root / "vggsound_full_best_av_af031_twoport_bm_log.md").write_text(text, encoding="utf-8")
...
def append_status(root: Path, results: List[Dict]) -> None:
    ...
    lines = [
        "",
        "## Current Best AF031 Audio-Video Fusion",
        "",
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run two-port BM on AF031 audio and best same-dim VGGSound video features.")
    ...
    p.add_argument("--audio_npz", type=Path, default=Path("./data_vggsound_full/features/vggsound_full_audio_paperresnet50_lstm4096_chunks4_w500_h1024_seed123.npz"))
    p.add_argument("--only", type=str, default="", help="Comma-separated experiment IDs, e.g. AV012,AV013.")
...
def main() -> None:
    ...
    for exp in selected_experiments(args):
        ...
        write_log(root, feature_npz, results)
        append_status(root, results)
    write_log(root, feature_npz, results)
    append_status(root, results)
    print("AF031 audio-video two-port BM experiments finished.", flush=True)
