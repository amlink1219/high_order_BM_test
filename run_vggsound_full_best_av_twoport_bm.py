...
EXPERIMENTS: List[Dict] = [
    {
        "id": "AV001",
        "name": "standard_avg_videolstm4096_audiocnnlstm4096_h8_lc5_e320",
        "model_type": "standard",
    ...
    {
        "id": "AV002",
        "name": "twoport_videolstm4096_audiocnnlstm4096_h8_g115_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 8.0,
        "gamma_h": 1.15,
        "gamma_l": 1.15,
    ...
    {
        "id": "AV003",
        "name": "twoport_videolstm4096_audiocnnlstm4096_h8_g050_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 8.0,
        "gamma_h": 0.50,
        "gamma_l": 0.50,
    ...
    {
        "id": "AV004",
        "name": "twoport_videolstm4096_audiocnnlstm4096_h8_g000_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 8.0,
        "gamma_h": 0.0,
        "gamma_l": 0.0,
    ...
    {
        "id": "AV005",
        "name": "twoport_videolstm4096_audiocnnlstm4096_h6_g115_lc5_e320",
        "model_type": "twoport",
        "hidden_factor": 6.0,
        "gamma_h": 1.15,
        "gamma_l": 1.15,
...
def ensure_aligned_feature(root: Path, args: argparse.Namespace) -> Path:
    out_npz = root / "data_vggsound_full" / "features" / "vggsound_full_aligned_videolstm4096_audiocnnlstm4096_seed123.npz"
    ...
    print(f"\n[{now_text()}] ALIGN best video/audio features -> {out_npz}", flush=True)
    run_checked(
        cmd,
        root,
        root / "runs_vggsound_full_AV000_align_best_av_stdout.log",
        root / "runs_vggsound_full_AV000_align_best_av_stderr.log",
...
def train_one(root: Path, args: argparse.Namespace, exp: Dict, feature_npz: Path, info: Dict) -> Dict:
    input_dim = int(exp.get("input_dim_override", info["input_dim"]))
...
def write_log(root: Path, feature_npz: Path, results: List[Dict]) -> None:
    ...
    (root / "vggsound_full_best_av_twoport_bm_log.md").write_text(text, encoding="utf-8")
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Run two-port BM on current best video and audio VGGSound features.")
    ...
    p.add_argument("--audio_npz", type=Path, default=Path("./data_vggsound_full/features/vggsound_full_audio_cnnlstm4096_stft128x96_per_dim_zscore_sigmoid_seed123.npz"))
...
def main() -> None:
    ...
    write_log(root, feature_npz, results)
    append_status(root, results)
    print("Best audio-video two-port BM experiments finished.", flush=True)
