...
H8_EXPERIMENTS: List[Dict] = [
    {
        "id": "AF020",
        "name": "standard_audio_cnnlstm4096_h8_lc5_e300",
        "epochs": 300,
        "resume_from": "",
    },
    {
        "id": "AF021",
        "name": "standard_audio_cnnlstm4096_h8_lc5_e500_resume_af020",
        "epochs": 500,
        "resume_from": "runs_vggsound_full_AF020_standard_audio_cnnlstm4096_h8_lc5_e300",
    },
]
...
def prepare_resume(out_dir: Path, resume_dir: Path, force_train: bool) -> None:
...
def train_one(root: Path, args: argparse.Namespace, exp: Dict, feature_npz: Path, num_classes: int) -> Dict:
    ...
    if exp["resume_from"]:
        ...
        prepare_resume(out_dir, resume_dir, args.force_train)
    ...
    print(
        f"\n[{now_text()}] TRAIN H8 {exp['id']} to epoch {exp['epochs']} hidden={hidden_dim} total={total_pbits}",
        flush=True,
    )
...
def update_status(root: Path, results: List[Dict]) -> None:
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train larger hidden 8x BM on CNN-LSTM4096 audio features.")
    ...
    p.add_argument("--only_e500", action="store_true")
...
def main() -> None:
    ...
    experiments = H8_EXPERIMENTS
    if args.only_e300:
        experiments = [e for e in experiments if e["id"] == "AF020"]
    if args.only_e500:
        experiments = [e for e in experiments if e["id"] == "AF021"]
    results: List[Dict] = []
    ...
    update_status(root, results)
    print("CNN-LSTM4096 h8 BM training finished.", flush=True)
