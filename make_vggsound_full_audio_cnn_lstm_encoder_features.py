...
class AudioCnnBiLstmEncoder(nn.Module):
    def __init__(
        self,
        embedding_dim: int,
        num_classes: int,
        ...
        if n_freq % 8 != 0:
            raise ValueError(f"n_freq={n_freq} must be divisible by 8 for the current CNN pooling stack")
        reduced_freq = n_freq // 8
        frame_dim = c3 * reduced_freq
        self.frame_proj = nn.Sequential(
            nn.Linear(frame_dim, lstm_input_dim),
    ...
    def sequence_features(self, x: torch.Tensor) -> torch.Tensor:
        z = self.conv(x)
        # z: [B, C, F', T]. Preserve temporal order for the LSTM.
        z = z.permute(0, 3, 1, 2).contiguous()
        z = z.flatten(2)
        return self.frame_proj(z)
...
def save_teacher_checkpoint(path: Path, model: AudioCnnBiLstmEncoder, optimizer, epoch: int, best_top1: float, args) -> None:
...
def train_encoder(args, device: torch.device):
    ...
    if bool(args.data_parallel and device.type == "cuda" and torch.cuda.device_count() > 1):
        print(f"[data_parallel] using {torch.cuda.device_count()} GPUs for CNN-BiLSTM teacher", flush=True)
...
@torch.no_grad()
def compute_embeddings(model: AudioCnnBiLstmEncoder, x: np.ndarray, batch_size: int, n_freq: int, n_time: int, device: torch.device) -> np.ndarray:
    model.eval()
    core_model = model.module if isinstance(model, nn.DataParallel) else model
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train a full VGGSound CNN-BiLSTM audio encoder and export embeddings for BM.")
    ...
    p.add_argument("--experiment_id", type=str, default="full_audio_cnn_lstm_encoder")
    p.add_argument("--n_freq", type=int, default=128)
    p.add_argument("--n_time", type=int, default=96)
    p.add_argument("--embedding_dim", type=int, default=2048)
...
def main() -> None:
    ...
    print(f"[{now_text()}] train full audio CNN-BiLSTM encoder device={device} audio_npz={args.audio_npz}", flush=True)
    ...
    summary = {
        ...
        "note": "Embeddings are supervised CNN-BiLSTM audio features from STFT inputs. Final BM eval uses train_vggsound_mini20_bm.py with input_mode=audio.",
    }
