...
class AudioCnnEncoder(nn.Module):
    def __init__(self, embedding_dim: int, num_classes: int, dropout: float, width: int = 32) -> None:
        super().__init__()
        c1 = width
        c2 = width * 2
        c3 = width * 4
        ...
        self.proj = nn.Sequential(
            nn.Flatten(),
            nn.Linear(c4 * 4 * 4, embedding_dim),
...
def save_teacher_checkpoint(path: Path, model: AudioCnnEncoder, optimizer, epoch: int, best_top1: float, args) -> None:
    torch.save(
        {
...
def train_encoder(args, device: torch.device):
    ...
    model = AudioCnnEncoder(args.embedding_dim, num_classes, args.dropout, width=args.cnn_width).to(device)
...
@torch.no_grad()
def compute_embeddings(model: AudioCnnEncoder, x: np.ndarray, batch_size: int, n_freq: int, n_time: int, device: torch.device) -> np.ndarray:
    model.eval()
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train a full VGGSound audio CNN encoder and export embeddings for BM.")
    ...
    p.add_argument("--experiment_id", type=str, default="full_audio_cnn_encoder")
...
def main() -> None:
    ...
    print(f"[{now_text()}] train full audio CNN encoder device={device} audio_npz={args.audio_npz}", flush=True)
    ...
    summary = {
        ...
        "note": "Embeddings are supervised audio CNN features from STFT inputs. Final BM eval uses train_vggsound_mini20_bm.py with input_mode=audio.",
    }
