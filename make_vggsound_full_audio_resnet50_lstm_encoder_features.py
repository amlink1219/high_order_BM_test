...
def train_encoder(args, device: torch.device):
    ...
    train_ds = AudioSequenceDataset(seq_train, y_train, frame_mu, frame_sd)
    test_ds = AudioSequenceDataset(seq_test, y_test, frame_mu, frame_sd)
    ...
    if bool(args.data_parallel and device.type == "cuda" and torch.cuda.device_count() > 1):
        print(f"[data_parallel] using {torch.cuda.device_count()} GPUs for audio ResNet50-LSTM teacher", flush=True)
...
def compute_embeddings(model: nn.Module, seq: np.ndarray, y: np.ndarray, norm: Dict, batch_size: int, device: torch.device) -> np.ndarray:
    model.eval()
    core_model = model.module if isinstance(model, nn.DataParallel) else model
    ds = AudioSequenceDataset(seq, y, norm["frame_mu"], norm["frame_sd"])
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Train audio BiLSTM encoder from audio ResNet50 chunk sequence features.")
    ...
    p.add_argument("--experiment_id", type=str, default="ARF003")
...
def main() -> None:
    ...
    print(f"[{now_text()}] train audio ResNet50-LSTM encoder device={device} seq_npz={args.seq_npz}", flush=True)
    ...
    summary = {
        ...
        "note": "Embeddings are supervised audio BiLSTM features from audio ResNet50 temporal chunk sequence features. Final BM eval uses train_vggsound_mini20_bm.py with input_mode=audio.",
    }
