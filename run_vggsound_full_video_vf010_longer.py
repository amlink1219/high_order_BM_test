...
EXPERIMENTS: List[Dict] = [
    {
        "id": "VF014",
        "name": "video_resnet50_meanstd_f16_h8_lc5_resume360",
        "epochs": 360,
        "resume_from": "runs_vggsound_full_VF010_video_resnet50_meanstd_f16_h8_lc5_resume240",
    },
    {
        "id": "VF015",
        "name": "video_resnet50_meanstd_f16_h8_lc5_resume480",
        "epochs": 480,
        "resume_from": "runs_vggsound_full_VF014_video_resnet50_meanstd_f16_h8_lc5_resume360",
    },
]
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Continue VF010 video ResNet50 mean/std h8 BM to longer epochs.")
    p.add_argument("--root", type=str, default=".")
    p.add_argument("--feature_npz", type=Path, default=Path("./data_vggsound_full/features/vggsound_full_visual_motion_resnet50_meanstd_allclasses_f16_s224.npz"))
    p.add_argument("--python_bin", type=str, default=sys.executable)
    p.add_argument("--force_train", action="store_true")
    p.add_argument("--dry_run", action="store_true")
    p.add_argument("--only_e360", action="store_true")
    p.add_argument("--only_e480", action="store_true")
...
def main() -> None:
    ...
    if args.only_e360:
        experiments = [e for e in experiments if e["id"] == "VF014"]
    if args.only_e480:
        experiments = [e for e in experiments if e["id"] == "VF015"]
    results: List[Dict] = []
    ...
    append_status(root, results)
    print("VF010 longer continuation finished.", flush=True)
